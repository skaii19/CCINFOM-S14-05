...
class TournamentController:
    ...
    def save_new_tournament(self, data):
        try:
            ...
            if ttype.lower() == "masters":
                ...
                # Get all Champions tournaments in this year
                existing_ids = self.tournament_model.get_tournaments_by_type_and_year("Champions", year)
                
                conflict = False
                for existing_id in existing_ids:
                    # Get all locations for this existing Champions tournament
                    locations = self.location_model.get_locations(existing_id)  # should return list of dicts
                    for loc in locations:
                        if loc.get("country", "").lower() == country.lower():
                            conflict = True
                            break
                    if conflict:
                        break
                ...
                if conflict:
                    QMessageBox.warning(None, "Error", f"A Champions tournament already exists in {country} for {year}")
                    return

                # If no conflict, assign ID (only 1 Champions per year, ID fixed as C1)
                tid = f"{year}_C1"
    ...
    def edit_tournament(self, tournament_id):
        if not tournament_id:
            print("DEBUG: No tournament selected")
            return  # Exit if nothing is selected

        tournament = self.tournament_model.get_tournament(tournament_id)
        if tournament is None:
            print(f"DEBUG: Tournament ID '{tournament_id}' not found in DB")
            return

        location = self.location_model.get_location(tournament_id) or {}
    ...
    def save_edit_tournament(self, tid, data):
        try:
            ...
            self.tournament_model.update_tournament(tid, name, ttype, prize, start, end)
            ...
            QMessageBox.critical(None, "Error", f"Failed to update tournament: {e}")

    # ----------------- Delete Tournament -----------------
    def delete_tournament(self, tournament_id):
        if not tournament_id:
            QMessageBox.warning(None, "Error", "No tournament selected.")
            return

        # Fetch name from DB
        tournament = self.tournament_model.get_tournament(tournament_id)
        if not tournament:
            QMessageBox.warning(None, "Error", "Tournament not found.")
            return
        ...
        confirm = QMessageBox.question(
            None,
            "Delete Tournament",
            f"Are you sure you want to delete {tname}?",
