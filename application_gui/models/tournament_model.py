...
class TournamentModel:
    ...
    # Returns a list of tournament_ids for a given type and year.
    def get_tournaments_by_type_and_year(self, tournament_type, year):
        try:
            cur = get_cursor()
            cur.execute("""
                SELECT tournament_id
                FROM tournament
                WHERE tournament_type = %s AND YEAR(start_date) = %s
            """, (tournament_type, year))
            rows = cur.fetchall()
            # Return a list of just the IDs
